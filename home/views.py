...
def buscar(request):
    ...
    if x is None or not x:
        messages.add_message(request,message.INFO,'Digite um valor valido')
        return render(request,'home/index.html')

    dados = Games.objects.order_by('titulo').filter(
        Q(titulo__icontains=x) | Q(descricao__icontains=x) | Q(genero__icontains=x)
    )
